...
## Key Features
- **Hybrid Optimization Engine:** A sophisticated, two-phase solver combining Dynamic MRV (Variable Ordering) with Iterative Repair (Min-Conflicts). It intelligently monitors overlaps across staggered class schedules and multi-class groupings, providing high-performance resolution of complex constraints while strictly enforcing class-specific timing overrides and curriculum subject limits. The engine incorporates a rich pedagogical cost matrix to optimize teacher well-being (gap/window reduction), student fatigue (subject variety protection), and resource efficiency (automatic unique home room mapping with specialist fallback).
...
- **Intelligent Dashboard:** A centralized control center with optimized spatial hierarchy, featuring integrated profile management, granular class-specific timing overrides, real-time health metrics (including teacher workload saturation and over-allocation alerts), and a real-time, persistent activity feed for comprehensive session and data oversight.
