...
describe('timeUtils - getEffectiveDuration', () => {
  ...
  it('should use global defaults when class overrides are missing', () => {
    ...
    expect(getEffectiveDuration(classGroup as ClassGroup, globalSettings as Settings, 'CLASS')).toBe(40);
    expect(getEffectiveDuration(classGroup as ClassGroup, globalSettings as Settings, 'BREAK')).toBe(15);
    expect(getEffectiveDuration(classGroup as ClassGroup, globalSettings as Settings, 'LUNCH')).toBe(45);
  });
  ...
  it('should prioritize class overrides over global defaults', () => {
    ...
    expect(getEffectiveDuration(classGroup as ClassGroup, globalSettings as Settings, 'CLASS')).toBe(50);
    expect(getEffectiveDuration(classGroup as ClassGroup, globalSettings as Settings, 'BREAK')).toBe(20);
    expect(getEffectiveDuration(classGroup as ClassGroup, globalSettings as Settings, 'LUNCH')).toBe(60);
  });
  ...
  it('should return a fallback value if neither is defined', () => {
    ...
    expect(getEffectiveDuration(emptyClass as ClassGroup, emptyGlobal as Settings, 'CLASS')).toBe(40); // Default fallback
  });
});
