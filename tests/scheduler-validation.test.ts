...
describe("Scheduler Validation", () => {
  ...
  it("should enforce single resource subject constraint", () => {
    ...
    const state = initializeState(data);
    const result = checkSlotValidity(data, 0, 0, "t1", "c1", "s1_res", state);
  ...
  it("should enforce joint class integrity", () => {
    const data: AppData = {
      ...baseData,
      jointClasses: [{ id: "jc1", name: "Joint Math", subjectId: "s1", classIds: ["c1", "c2"] }],
    };

    const state = initializeState(data);
    const result = checkSlotValidity(data, 0, 0, "t1", "c1", "s1", state);
  ...
  it("should enforce room capacity constraint", () => {
    ...
    const state = initializeState(data);
    const result = checkSlotValidity(data, 0, 0, "t1", "c1", "s1", state, undefined, "r_small");
      ...
      schedule: {
        c2: {
          0: {
            // Monday
            0: {
              // Period 0 for Class B (08:00 - 09:00)
              subjectId: "s2",
              teacherId: "t1",
              classId: "c2",
            },
