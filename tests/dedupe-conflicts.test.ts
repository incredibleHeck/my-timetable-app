...
describe("dedupeConflicts", () => {
  ...
  it("merges vfs-style and crdb-style teacher double-book per class (2 not 4)", () => {
    ...
    const teacherRaw = raw.filter((c) => conflictDedupeKey(c).startsWith("teacher:t1:0:0:"));
    const teacherMerged = merged.filter((c) => conflictDedupeKey(c).startsWith("teacher:t1:0:0:"));
  ...
  it("prefers HIGH severity and longer reason when merging", () => {
    ...
    const high: Conflict = {
      ...low,
      reason: "Double Booking: Teacher Mr Smith is assigned to multiple classes (c1, c2)",
      severity: "HIGH",
    };
