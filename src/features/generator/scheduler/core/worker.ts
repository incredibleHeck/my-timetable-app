...
ctx.onmessage = (e: MessageEvent<AppData>) => {
  ...
  try {
    ...
    solverConflicts.forEach((c) => {
      ...
      if (c.classId && c.subjectId)
        reportedGaps.add(`${c.classId}-${c.subjectId}`);
    });
    ...
    audit.curriculumGaps.forEach((gap: any) => {
      ...
      if (!reportedGaps.has(key)) {
        finalConflicts.push({
          ...
          subjectName:
            data.subjects.find((s: any) => s.id === gap.subjectId)?.name ||
            gap.subjectId,
    ...
    const allConflicts = [...finalConflicts, ...logicalConflicts];
