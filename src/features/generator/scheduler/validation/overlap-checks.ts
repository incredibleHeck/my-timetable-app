...
export const checkOverlaps = (
  ...
  const { data, classId, teacherId, roomId, targetDay, ignoredSlots } = ctx;
      ...
      if (teacherOccupant !== currentUnitId) {
        const isJoint = data.jointClasses?.some(
          (jc) =>
            ...
            jc.classIds.some(
              (cid) => state.classOccupancy[cid]?.[targetDay]?.[p] === teacherOccupant,
            ),
        );
        if (!isJoint) {
          const otherCls = data.classes.find(
            (c) => state.classOccupancy[c.id]?.[targetDay]?.[p] === teacherOccupant,
          );
          return {
            valid: false,
  ...
  if (roomId) {
    ...
    if (roomOccupant && roomOccupant !== "BLOCK" && !ignoredSlots.has(`${targetDay}-${p}`)) {
      ...
      if (roomOccupant !== currentUnitId) {
        ...
        const otherCls = data.classes.find(
          (c) => state.classOccupancy[c.id]?.[targetDay]?.[p] === roomOccupant,
        );
        return {
          valid: false,
