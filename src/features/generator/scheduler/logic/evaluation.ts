...
import {
  ...
  buildEvictionIgnoredSlots,
  computeTeacherLoadAdjustment,
} from "./constraints";
...
export class EvaluationEngine {
  ...
  evaluate(
    ...
    const score = calculateScore(state, data, move.d, move.p, unit, teacherMap, subjectMap);
  ...
  evaluateMove(
    ...
    const p2 = unit.duration === 2 ? (getNextClassPeriod(p, struct, classLimit) ?? -1) : -1;

    if (unit.duration === 2 && p2 === -1) {
      return {
        isLegal: false,
        totalCost: Infinity,
        conflicts: ["No valid second period for double"],
      };
    }
    ...
    const teacherLoadAdjustment = computeTeacherLoadAdjustment(state, d, p, p2, victims, unitMap);
    ...
    if (victims.size > 0) {
      victims.forEach((vId) => {
        ...
        if (vUnit) {
          ...
          const isVComplex =
            vUnit.classIds.length > 1 || vUnit.jointClassId || vUnit.electiveBlockId;
    ...
    const targetRoomId = forceDetermineRoom(
      d,
      p,
      p2,
      unit,
