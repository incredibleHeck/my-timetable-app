...
export const checkSlotValidity = (
  ...
  while (periodsConsumed < duration) {
    ...
    if (globalFixed) {
      const label = typeof globalFixed === "object" && "name" in globalFixed ? globalFixed.name : "School Event";
        ...
        if (slot && slot.teacherId === teacherId) {
          ...
          if (targetTimeRange && otherTimeRange && doTimeRangesOverlap(targetTimeRange, otherTimeRange)) {
                ...
                const isJointOverlap = data.jointClasses?.some(jc => 
                  jc.subjectId === subjectId && jc.classIds.includes(classId) && jc.classIds.includes(cId)
                );
                const isElectiveOverlap = data.electives?.some(e => 
                  e.classIds.includes(classId) && e.classIds.includes(cId)
                );
    ...
    if (roomId) {
      ...
      if (room && cls && (cls.studentCount || 0) > room.capacity) {
        return { valid: false, message: `Room capacity (${room.capacity}) exceeded by ${cls.name}`, severity: "MEDIUM" };
      }
    }
  ...
  if (ignoreSlot && String(targetDay) === String(ignoreSlot.day)) {
      ...
      const slotAtIgnore = daySchedule[String(startP)] || daySchedule[startP];
      ...
      while (c < srcDur && (startP + o) < maxPeriods) {
          const p = startP + o;
          const type = (typeof structure?.[p] === "object" ? structure[p].type : structure?.[p]) || "CLASS";
          if (type === "CLASS") { ignoredSourceSlots.add(p); c++; }
          o++;
      }
  }
  ...
  if (ignoreTargetSlot && String(targetDay) === String(ignoreTargetSlot.day)) {
      let c = 0, o = 0;
      while (c < ignoreTargetSlot.duration && (ignoreTargetSlot.period + o) < maxPeriods) {
          const p = ignoreTargetSlot.period + o;
          const type = (typeof structure?.[p] === "object" ? structure[p].type : structure?.[p]) || "CLASS";
          if (type === "CLASS") { ignoredTargetSlots.add(p); c++; }
          o++;
      }
  }
  ...
  for (let checkP = 0; checkP < maxPeriods; checkP++) {
      ...
      if (proposedSlots.has(checkP)) {
           ...
           for (const cId of Object.keys(schedule)) {
              ...
              if (s && s.teacherId === teacherId) {
                  if (cId === classId && (ignoredSourceSlots.has(checkP) || ignoredTargetSlots.has(checkP))) continue;
                  isBusy = true;
                  break;
              }
          }
      ...
      if (isBusy) {
          ...
          consecutiveCount++;
          if (consecutiveCount > maxConsecutive) return { valid: false, message: `${teacher?.name} would exceed consecutive period limit (${maxConsecutive})`, severity: "MEDIUM" };
      } else { consecutiveCount = 0; }
  }
  if (busyPeriods.size > maxDailyLoad) return { valid: false, message: `Exceeds ${teacher?.name}'s daily limit of ${maxDailyLoad} periods`, severity: "MEDIUM" };

  // 5. JOINT CLASS INTEGRITY
  const isJoint = data.jointClasses?.some(jc => jc.subjectId === subjectId && jc.classIds.includes(classId));
  if (isJoint && !isAuto) return { valid: false, message: "Cannot move Joint Class manually (breaks sync)", severity: "HIGH" };

  // 6. CLASS-LEVEL SUBJECT CONSTRAINTS
  let existingPeriods = proposedSlots.size;
  let firstP = 999, lastP = -999;
  proposedSlots.forEach(p => { firstP = Math.min(firstP, p); lastP = Math.max(lastP, p); });

  for (const pStr in daySchedule) {
      const pIdx = parseInt(pStr);
      if (ignoredSourceSlots.has(pIdx) || ignoredTargetSlots.has(pIdx) || proposedSlots.has(pIdx)) continue;
  ...
  if (existingPeriods > maxSubjPeriods) return { valid: false, message: `Max ${maxSubjPeriods} periods of ${subjects.find(s => s.id === subjectId)?.name} per day`, severity: "MEDIUM" };

  if (existingPeriods > 1) {
      for (let gapP = firstP + 1; gapP < lastP; gapP++) {
          const type = (typeof structure?.[gapP] === "object" ? structure[gapP].type : structure?.[gapP]) || "CLASS";
          if (type === "CLASS") {
              const isOccupied = proposedSlots.has(gapP) || (daySchedule[gapP]?.subjectId === subjectId && !ignoredSourceSlots.has(gapP) && !ignoredTargetSlots.has(gapP));
              if (!isOccupied) return { valid: false, message: `Gap detected in ${subjects.find(s => s.id === subjectId)?.name} sessions`, severity: "MEDIUM" };
          }
      }
  }
  ...
  if (targetSlot) {
    if ((targetSlot as any).locked) return { valid: false, message: "Target slot is Locked", severity: "HIGH" };
    if (targetSlot.electiveBlockId) return { valid: false, message: "Cannot move Elective Block manually", severity: "HIGH" };
  ...
  if (duration === 2) {
      ...
      while (nextP < maxPeriods) {
          const type = (typeof structure?.[nextP] === "object" ? structure[nextP].type : structure?.[nextP]) || "CLASS";
          if (type === "CLASS") break;
          nextP++;
      }
      if (nextP < maxPeriods && schedule[classId]?.[targetDay]?.[nextP]) return { valid: true, isSwap: true, message: "Swap with existing lesson (P2)" };
  }
  ...
  for (const classId of Object.keys(schedule)) {
    ...
    for (const dayStr of Object.keys(schedule[classId])) {
      ...
      for (const periodStr of Object.keys(daySchedule)) {
        ...
        const result = checkSlotValidity(data, day, period, slot.teacherId, classId, slot.subjectId, { day, period, duration }, slot.roomId, duration, undefined, true);
        if (!result.valid) {
          const subject = subjects.find(s => s.id === slot.subjectId);
          const teacher = teachers.find(t => t.id === slot.teacherId);
          allConflicts.push({ classId, className: cls.name, subjectId: slot.subjectId, subjectName: subject?.name || "Unknown", teacherName: teacher?.name || "Unknown", day, period, reason: result.message || "Invalid placement", severity: result.severity || "HIGH" });
        }
      }
    }
  }
