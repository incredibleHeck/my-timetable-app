...
export const QuickAction: React.FC<QuickActionProps> = ({ icon, label, desc, onClick }) => (
  ...
  >
    <div className="p-3 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-300 rounded-lg group-hover:bg-amber-50 dark:bg-amber-900/30 group-hover:text-accent-ink transition-colors">
      {icon}
    </div>
    <div className="flex-1">
      <h4 className="font-bold text-slate-700 dark:text-slate-200 text-sm group-hover:text-amber-800 dark:text-amber-200 transition-colors">
        {label}
      </h4>
