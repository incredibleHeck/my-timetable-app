...
export const useClassMetrics = (data: AppData) => {
  // Memoised so callers can list it as a dependency: it derives entirely from
  // `data.settings`, and a fresh identity every render forced consumers to
  // silence the exhaustive-deps rule instead.
